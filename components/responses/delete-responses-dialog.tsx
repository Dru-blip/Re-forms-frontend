...
import * as responseActions from "@/lib/actions/responses";

export default function DeleteResponseDialog({ formId }: { formId: string }) {
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const onClick = async () => {
        setIsLoading(true);
        const res = await responseActions.deleteResponses(formId);
        if (res.msg === "error") {
    ...
    return (
        <AlertDialog>
            <AlertDialogTrigger asChild>
                <Button>
                    <Trash2 className="mr-2 w-5 h-5" /> Delete All Responses
                </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
                <AlertDialogHeader>
